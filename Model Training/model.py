...
class Postnet(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        for l in range(self.n_conv_blocks):
            ...
            y = self.act(y)
            y = self.norm_layer(y)
            y = self.append_cond_2d(y, self.conv_affine_layers[l*2](cond))
            y = self.pad_layer_2d(y, self.second_conv_layers[l])
            y = self.act(y)
            if self.upsample[l] > 1:
                y = self.ps(y)
                y = self.norm_layer(y)
                y = self.append_cond_2d(y, self.conv_affine_layers[l*2+1](cond))
